...
func TestAdaptiveIndex(t *testing.T) {
	...
	ds := NewMockDataset("test_adaptive", schema)
	...
	results, err := idx.SearchVectors(context.Background(), []float32{1, 0, 0, 0}, 1, nil, types.SearchOptions{})
	require.NoError(t, err)
...
func TestAdaptiveIndexDisabled(t *testing.T) {
	...
	ds := NewMockDataset("test_disabled", nil)
	cfg := AdaptiveIndexConfig{
		Threshold: 1,
		Enabled:   false,
	}
