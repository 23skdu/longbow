...
headerBuf := make([]byte, 12) // 4 bytes uint32 + 8 bytes uint64
binary.LittleEndian.PutUint32(headerBuf[0:4], nameLen)
binary.LittleEndian.PutUint64(headerBuf[4:12], recLen)

// Verify decoding roundtrip
decodedNameLen := binary.LittleEndian.Uint32(headerBuf[0:4])
decodedRecLen := binary.LittleEndian.Uint64(headerBuf[4:12])
...
{"zero values", 0, 0},
{"small values", 10, 100},
{"medium values", 1000, 1000000},
{"large name", 0xFFFFFFFF, 100},
{"large record", 100, 0xFFFFFFFFFFFFFFFF},
}

for _, tt := range tests {
t.Run(tt.name, func(t *testing.T) {
buf := encodeWALEntryHeader(tt.nameLen, tt.recLen)
...
gotNameLen := binary.LittleEndian.Uint32(buf[0:4])
gotRecLen := binary.LittleEndian.Uint64(buf[4:12])
...
b.ResetTimer()
for i := 0; i < b.N; i++ {
_ = encodeWALEntryHeader(nameLen, recLen)
}
}
