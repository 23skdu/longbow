...
func TestShardedHNSW_Compaction(t *testing.T) {
	...
	dataset := &MockDataset{Name: "test-compaction"}
	...
	// Verify lookups
	res, err := idx.SearchVectors(context.Background(), []float32{0, 0, 0, 0}, 1, nil, types.SearchOptions{})
	require.NoError(t, err)
	require.Len(t, res, 1)
...
func TestShardedHNSW_Vacuum(t *testing.T) {
	...
	mem := memory.NewGoAllocator()
	dataset := &MockDataset{Name: "test-vacuum"}
	config := DefaultShardedHNSWConfig()
	config.NumShards = 1
	config.Dimension = 4
	idx := NewShardedHNSW(config, dataset).(*ShardedHNSW)
	...
	// Since we mock deletion via types.IndexDataProvider Tombstones, but ArrowHNSW uses `Index.Tombstones`?
...
func TestShardedHNSW_DynamicGrowth(t *testing.T) {
	...
	mem := memory.NewGoAllocator()
	dataset := &MockDataset{Name: "test-growth"}
	config := DefaultShardedHNSWConfig()
