...
// =============================================================================
// Subtask 5: DoGet Hybrid Search API Tests
// =============================================================================
...
func TestHybridResultCombination(t *testing.T) {
	...
	combined := CombineHybridResults(vectorResults, bm25Results, alpha, 5)

	if len(combined) == 0 {
		t.Error("expected combined results")
	}

	// ID 1 and 2 appear in both, should be ranked higher
	// due to appearing in both result sets
	for i, r := range combined {
		t.Logf("Rank %d: ID=%d Score=%f", i, r.ID, r.Score)
	}
}
...
func TestHybridResultRRF(t *testing.T) {
	...
	// RRF with k=60: score = 1/(k+rank)
	// ID 1: 1/(60+1) + 1/(60+2) = 0.0164 + 0.0161 = 0.0325
	// ID 2: 1/(60+2) + 1/(60+1) = 0.0161 + 0.0164 = 0.0325
	// Should be tied
