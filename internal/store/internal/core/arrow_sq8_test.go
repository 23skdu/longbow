...
func TestSQ8Indexing(t *testing.T) {
	...
	ds := NewMockDataset("test_sq8", schema)
	...
	if len(data.VectorsSQ8) > 0 {
		...
		if chunk != nil {
			...
			assert.Equal(t, types.ChunkSize*64, len(chunk))
		}
	}
	...
	targetID := ids[10]
	// Approximate search
	res, err := idx.SearchVectors(context.Background(), query, 10, nil, types.SearchOptions{})
	require.NoError(t, err)
