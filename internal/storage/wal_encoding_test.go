...
func TestEncodeWALEntryHeader(t *testing.T) {
	tests := []struct {
		...
		{"small values", 12345, 1, 1000000, 10, 100},
		{"medium values", 67890, 100, 2000000, 1000, 1000000},
		{"large name", 99999, 1000, 3000000, 0xFFFFFFFF, 100},
		{"large record", 88888, 10000, 4000000, 100, 0xFFFFFFFFFFFFFFFF},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			decodedCrc := binary.LittleEndian.Uint32(buf[0:4])
			decodedSeq := binary.LittleEndian.Uint64(buf[4:12])
			decodedTs := int64(binary.LittleEndian.Uint64(buf[12:20]))
			decodedNameLen := binary.LittleEndian.Uint32(buf[20:24])
			decodedRecLen := binary.LittleEndian.Uint64(buf[24:32])

			if decodedCrc != tt.crc {
				t.Errorf("crc: got %d, want %d", decodedCrc, tt.crc)
			}
			if decodedSeq != tt.seq {
				t.Errorf("seq: got %d, want %d", decodedSeq, tt.seq)
			}
			if decodedTs != tt.ts {
				t.Errorf("ts: got %d, want %d", decodedTs, tt.ts)
			}
			if decodedNameLen != tt.nameLen {
				t.Errorf("nameLen: got %d, want %d", decodedNameLen, tt.nameLen)
			}
			if decodedRecLen != tt.recLen {
				t.Errorf("recLen: got %d, want %d", decodedRecLen, tt.recLen)
			}
		})
	}
}
