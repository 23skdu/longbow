package core

import (
	"github.com/23skdu/longbow/internal/store/types"
	"testing"

	"github.com/apache/arrow-go/v18/arrow"
	"github.com/stretchr/testify/assert"
)

func TestChunkedGrowth(t *testing.T) {
	config := types.DefaultArrowHNSWConfig()
	config.InitialCapacity = 10 // Start small

	// Create HNSW
	fields := []arrow.Field{
		{Name: "vector", Type: arrow.FixedSizeListOf(128, arrow.PrimitiveTypes.Float32)},
	}
	schema := arrow.NewSchema(fields, nil)
	ds := NewMockDataset("test", schema)
	h := NewArrowHNSW(ds, &config)

	// Verify initial chunks
	data := h.data.Load()

	// Default min capacity is 1000 in NewArrowHNSW logic, so we expect 1 chunk (if types.ChunkSize=1024)
	// Let's verify expectations
	assert.GreaterOrEqual(t, data.Capacity, 1000)

	// We want to force growth.
	// Let's manually manipulate capacity for testing, OR just Grow huge.

	targetCap := 5000
	if err := h.Grow(targetCap, 128); err != nil {
		t.Fatalf("Grow failed: %v", err)
	}

	newData := h.data.Load()
	assert.GreaterOrEqual(t, newData.Capacity, targetCap)

	// Check num chunks
	// Grow adds types.ChunkSize headroom and then rounds up
	// Grow ensures capacity >= targetCap. It rounds up to types.ChunkSize.
	// We expect capacity to be 5000 (if exact) rounded up to types.ChunkSize.
	// 5000 / 1024 = 4.88 -> 5 chunks.
	expectedCap := targetCap
	if expectedCap%types.ChunkSize != 0 {
		expectedCap = ((expectedCap / types.ChunkSize) + 1) * types.ChunkSize
	}
	expectedChunks := expectedCap / types.ChunkSize
	assert.Equal(t, expectedChunks, len(newData.Levels))

	// Ensure Chunk 0 is allocated for testing
	// Use ensureChunk which handles Arena allocation
	var err error
	newData, err = h.ensureChunk(newData, 0, 0, 128)
	if err != nil {
		t.Fatal(err)
	}

	// Test Data Persistence across Grow
	// Write something to chunk 0
	cID := types.ChunkID(0)
	cOff := types.ChunkOffset(0)
	levels := newData.GetLevelsChunk(cID)
	if levels != nil {
		levels[cOff] = 99
	}
	neighbors := newData.GetNeighborsChunk(0, cID)
	if neighbors != nil {
		neighbors[int(cOff)*types.MaxNeighbors] = 999
	}

	// Grow again
	if err := h.Grow(targetCap+types.ChunkSize, 128); err != nil {
		t.Fatalf("Grow failed: %v", err)
	} // Add one more chunk
	grownData := h.data.Load()

	// Check old data preserved
	gLevels2 := grownData.GetLevelsChunk(0)
	if gLevels2 == nil || len(gLevels2) <= int(cOff) || gLevels2[cOff] != 99 {
		t.Error("Level data lost after grow")
	}
	gNeighbors2 := grownData.GetNeighborsChunk(0, 0)
	if gNeighbors2 == nil || len(gNeighbors2) <= int(cOff)*types.MaxNeighbors || gNeighbors2[int(cOff)*types.MaxNeighbors] != 999 {
		t.Error("Neighbor data lost after grow")
	}
}

func TestChunkHelpers(t *testing.T) {
	// Verify types.ChunkID/Offset logic matches types.ChunkSize constants
	id := uint32(types.ChunkSize + 5)

	cID := types.ChunkID(id)
	cOff := types.ChunkOffset(id)

	assert.Equal(t, 1, cID)
	assert.Equal(t, 5, cOff)

	id2 := uint32(5)
	assert.Equal(t, 0, types.ChunkID(id2))
	assert.Equal(t, 5, types.ChunkOffset(id2))
}
