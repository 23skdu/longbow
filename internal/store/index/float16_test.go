...
func TestFloat16_RoundTrip(t *testing.T) {
	...
	gd := lbtypes.NewGraphData(capacity, dims, false, false, 0, false, true, false, lbtypes.VectorTypeFloat16, false, false, false, 8, "test", nil, false)
	...
	id := uint32(1)
	cID := lbtypes.ChunkID(id)
	cOff := lbtypes.ChunkOffset(id)
...
func TestArrowHNSW_Float16_Integration(t *testing.T) {
	...
	dims := 128
	cfg := lbtypes.DefaultArrowHNSWConfig()
	cfg.Dims = dims
	...
	for i := 0; i < n; i++ {
		...
		// Since we are in package core, we can access unexported methods.

		retrieved, err := hnsw.GetVectorAny(id)
		require.NoError(t, err)
