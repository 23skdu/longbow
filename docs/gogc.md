...
The Go runtime's default GC policy (`GOGC=100`) triggers a collection when the heap grows by 100% relative to the live heap size. This works well generally but can leave a significant amount of memory unused on systems with large RAM, or cause Out-Of-Memory (OOM) errors if the heap spikes.

Longbow's **GCTuner** dynamically adjusts `GOGC` based on the current heap usage relative to a configured **soft memory limit**.

- **Low Memory Usage**: Increases `GOGC` (e.g., to 100-500). This reduces GC frequency, saving CPU cycles for application logic.
- **High Memory Usage**: Decreases `GOGC` (e.g., to 10-50). This forces more frequent GCs to reclaim memory aggressively and prevent OOMs.
...
func main() {
    ...
    // Initialize GC Tuner (e.g., 4GB limit)
    limit := int64(4 * 1024 * 1024 * 1024)
    tuner := memory.NewGCTuner(limit, 100, 10, logger)
    
    // Start background monitoring (e.g., check every 1s)
    go tuner.Start(ctx, 1*time.Second)
