...
func TestBruteForceIndex_ZeroCopyVectorAccess(t *testing.T) {
	...
	t.Run("getVectorUnsafe_ReturnsDirectReference", func(t *testing.T) {
		...
		vec, release := idx.getVectorUnsafe(types.Location{BatchIdx: 0, RowIdx: 0})
		...
		// Get another vector
		vec2, release2 := idx.getVectorUnsafe(types.Location{BatchIdx: 0, RowIdx: 1})
	...
	t.Run("getVectorUnsafe_NoAllocation", func(t *testing.T) {
		// Warm up
		for i := 0; i < 100; i++ {
			vec, release := idx.getVectorUnsafe(types.Location{BatchIdx: 0, RowIdx: i})
		...
		for i := 0; i < 1000; i++ {
			vec, release := idx.getVectorUnsafe(types.Location{BatchIdx: 0, RowIdx: i % numVectors})
			if vec != nil && release != nil {
	...
	t.Run("getVector_CreatesAllocations", func(t *testing.T) {
		// Warm up
		for i := 0; i < 100; i++ {
			_ = idx.getVector(types.Location{BatchIdx: 0, RowIdx: i})
		}
		...
		for i := 0; i < 1000; i++ {
			vec := idx.getVector(types.Location{BatchIdx: 0, RowIdx: i % numVectors})
			_ = vec[0] // Use the vector
		}
...
func TestBruteForceIndex_SearchWithZeroCopy(t *testing.T) {
	...
	t.Run("SearchWithZeroCopy_ReducesAllocations", func(t *testing.T) {
		...
		// Perform searches
		for i := 0; i < 100; i++ {
			results, err := idx.SearchVectors(context.Background(), query, 10, nil, types.SearchOptions{})
			require.NoError(t, err)
