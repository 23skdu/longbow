...
// resultPool manages pools of []types.VectorID slices for common k values.
...
func newResultPool() *resultPool {
	return &resultPool{
		pool10: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 10)
				return &s
			},
		},
		pool20: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 20)
				return &s
			},
		},
		pool50: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 50)
				return &s
			},
		},
		pool100: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 100)
				return &s
			},
		},
		pool128: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 128)
				return &s
			},
		},
		pool256: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 256)
				return &s
			},
		},
		pool512: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 512)
				return &s
			},
		},
		pool1000: sync.Pool{
			New: func() any {
				s := make([]types.VectorID, 1000)
				return &s
			},
		},
	}
...
// get retrieves a []types.VectorID slice of the specified length.
...
func (p *resultPool) get(k int) []types.VectorID {
	var slice []types.VectorID

	switch {
	// Exact matches for pooled sizes
	case k == 10:
		ptr := p.pool10.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 20:
		ptr := p.pool20.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 50:
		ptr := p.pool50.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 100:
		ptr := p.pool100.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 128:
		ptr := p.pool128.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 256:
		ptr := p.pool256.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 512:
		ptr := p.pool512.Get().(*[]types.VectorID)
		slice = *ptr
	case k == 1000:
		ptr := p.pool1000.Get().(*[]types.VectorID)
		slice = *ptr
...
// put returns a []types.VectorID slice to the appropriate pool.
func (p *resultPool) put(slice []types.VectorID) {
	if slice == nil {
		return
	}
