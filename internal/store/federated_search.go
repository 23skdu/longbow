...
func (fqr *FederatedQueryRouter) mergeResultsBorda(partials []partialResult, k int) []lbtypes.SearchResult {
	type scoredResult struct {
		id         lbtypes.VectorID
		bordaScore float64
		collection string
	...
	collectionCount := len(partials)
	var allScored []scoredResult

	for _, p := range partials {
		for i, r := range p.results {
			rank := i + 1
			bordaScore := float64(collectionCount - rank)
			allScored = append(allScored, scoredResult{
				id:         r.ID,
	...
	for i := range allScored {
		s := &allScored[i]
		if existing, ok := scoreMap[s.id]; ok {
			existing.bordaScore += s.bordaScore
		} else {
			scoreMap[s.id] = s
		}
	}
	...
	sort.Slice(uniqueScored, func(i, j int) bool {
		return uniqueScored[i].bordaScore > uniqueScored[j].bordaScore
	})
	...
	for i := 0; i < limit; i++ {
		results[i] = lbtypes.SearchResult{
			ID:       uniqueScored[i].id,
			Distance: uniqueScored[i].distance,
			Score:    float32(uniqueScored[i].bordaScore),
			Metadata: map[string]interface{}{
				"collection": uniqueScored[i].collection,
			},
		}
...
func (fqr *FederatedQueryRouter) mergeResultsWeighted(partials []partialResult, weights map[string]float64, k int) []lbtypes.SearchResult {
	type scoredResult struct {
		id         lbtypes.VectorID
		score      float64
		collection string
	...
	for _, p := range partials {
		weight := weights[p.collection]
		if weight == 0 {
			weight = 1.0
		}
		for i, r := range p.results {
			rank := i + 1
			rrfScore := weight * (1.0 / (0.6 * float64(rank)))
			allScored = append(allScored, scoredResult{
				id:         r.ID,
				score:      rrfScore,
				collection: p.collection,
