...
func (h *ArrowHNSW) Search(q []float32, k, ef int, filter *query.Bitset) ([]SearchResult, error) {
	...
	if h.bqEncoder != nil {
		...
		fmt.Printf("DBG: Search encoded BQ: %v from q[0]=%v\n", ctx.queryBQ, q[0])
	}
	...
	for l := maxL; l > 0; l-- {
		ctx.Reset()
		ep = h.searchLayer(q, ep, 1, l, ctx, data, nil)
	}

	// 2. Final Search at Layer 0
	ctx.Reset()

	_ = h.searchLayer(q, ep, ef, 0, ctx, data, filter)
...
func (h *ArrowHNSW) searchLayer(q []float32, entryPoint uint32, ef, layer int, ctx *ArrowSearchContext, data *GraphData, filter *query.Bitset) (candidate uint32) {
	...
	useSQ8 := h.quantizer != nil && h.sq8Ready.Load() && len(data.VectorsSQ8) > 0
	useBQ := h.config.BQEnabled && len(data.VectorsBQ) > 0 && h.bqEncoder != nil
	usePQ := h.config.PQEnabled && h.pqEncoder != nil && len(data.VectorsPQ) > 0
	useF16 := h.config.Float16Enabled

	// Lazy encode query if not already done (e.g. from insert path)
	if useF16 && len(ctx.queryF16) == 0 {
	...
	if useSQ8 && len(ctx.querySQ8) == 0 {
		if cap(ctx.querySQ8) < len(q) {
			ctx.querySQ8 = make([]byte, len(q))
		}
		ctx.querySQ8 = ctx.querySQ8[:len(q)]
		h.quantizer.Encode(q, ctx.querySQ8)
	}
	if useBQ && len(ctx.queryBQ) == 0 && h.bqEncoder != nil {
		ctx.queryBQ = h.bqEncoder.Encode(q)
	}

	var sq8Scale float32
	if useSQ8 {
		sq8Scale = h.quantizer.L2Scale()
	}
	...
	case useSQ8 && h.metric == MetricEuclidean:
		vSQ8 := data.GetVectorsSQ8Chunk(chunkID(entryPoint))
		if vSQ8 != nil {
			dims := int(h.dims.Load())
			off := int(chunkOffset(entryPoint)) * dims
			entryDist = float32(simd.EuclideanDistanceSQ8(ctx.querySQ8, vSQ8[off:off+dims])) * sq8Scale
		} else {
			entryDist = h.distance(q, entryPoint, data, ctx)
		}
	case useBQ:
		vBQ := data.GetVectorsBQChunk(chunkID(entryPoint))
		if vBQ != nil {
			numWords := (int(h.dims.Load()) + 63) / 64
			off := int(chunkOffset(entryPoint)) * numWords
			dist := simd.HammingDistance(ctx.queryBQ, vBQ[off:off+numWords])
			entryDist = float32(dist)
		} else {
			entryDist = h.distance(q, entryPoint, data, ctx)
		}
	default:
	...
	for ctx.candidates.Len() > 0 {
		...
		case usePQ && len(adcTable) > 0:
			pqM := data.PQDims
			if cap(ctx.scratchPQCodes) < batchCount*pqM {
				ctx.scratchPQCodes = make([]byte, batchCount*pqM)
			}
			codes := ctx.scratchPQCodes[:batchCount*pqM]
			for i, nid := range ctx.scratchIDs {
				v := data.GetVectorPQ(nid)
				if v != nil {
					copy(codes[i*pqM:(i+1)*pqM], v)
				} else {
					for j := 0; j < pqM; j++ {
						codes[i*pqM+j] = 0
					}
			...
			h.pqEncoder.ADCDistanceBatch(adcTable, codes, dists)
		case useSQ8 && h.metric == MetricEuclidean:
			for i, nid := range ctx.scratchIDs {
				v := data.GetVectorSQ8(nid)
				if v != nil {
					dists[i] = float32(simd.EuclideanDistanceSQ8(ctx.querySQ8, v)) * sq8Scale
				} else {
					dists[i] = h.distance(q, nid, data, ctx)
				}
			}
		default:
...
func (h *ArrowHNSW) getVectorF16(id uint32) ([]float16.Num, error) {
	...
	v := data.GetVectorF16(id)
	if v == nil {
		return nil, fmt.Errorf("vector f16 not found: %d", id)
	}
	return v, nil
}
...
func (h *ArrowHNSW) mustGetVectorFromData(data *GraphData, id uint32) []float32 {
	...
	// 1. Try Float32 (High Precision)
	vecChunk := data.GetVectorsChunk(cID)
	if vecChunk != nil {
		return vecChunk[int(cOff)*dims : int(cOff+1)*dims]
	}

	// 2. Try Float16 (Lossy)
	f16Chunk := data.GetVectorsF16Chunk(cID)
	if f16Chunk != nil {
		f16s := f16Chunk[int(cOff)*dims : (int(cOff)+1)*dims]
		f32s := make([]float32, dims)
		for i, v := range f16s {
			f32s[i] = v.Float32()
		}
		return f32s
	...
	if sq8Chunk != nil && h.quantizer != nil {
		sq8s := sq8Chunk[int(cOff)*dims : (int(cOff)+1)*dims]
