...
// Create test chunk
chunk := createTestChunk(t, "test-dataset")
...
// Fill queue
chunk1 := createTestChunk(t, "ds1")
chunk2 := createTestChunk(t, "ds2")
chunk3 := createTestChunk(t, "ds3")
...
defer wg.Done()
for j := 0; j < 25; j++ {
chunk := createTestChunk(t, "concurrent")
...
// Enqueue work
for i := 0; i < 10; i++ {
chunk := createTestChunk(t, "stats-test")
queue.TryEnqueue(chunk)
}
...
chunk := createTestChunk(t, "error-test")
queue.TryEnqueue(chunk)
...
func createTestChunk(t *testing.T, datasetName string) *FlightDataChunk {
t.Helper()
