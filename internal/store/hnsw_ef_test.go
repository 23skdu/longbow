...
func TestHNSW_EfParameter(t *testing.T) {
	...
	t.Run("DefaultEf", func(t *testing.T) {
		...
		res, err := idx.SearchVectors(context.Background(), query, k, nil, SearchOptions{})
		require.NoError(t, err)
		require.Len(t, res, k)
	...
	t.Run("CustomEf_Larger", func(t *testing.T) {
		...
		res, err := idx.SearchVectors(context.Background(), query, k, nil, SearchOptions{Ef: 100})
		require.NoError(t, err)
		require.Len(t, res, k)
	...
	t.Run("CustomEf_Smaller", func(t *testing.T) {
		...
		res, err := idx.SearchVectors(context.Background(), query, k, nil, SearchOptions{Ef: 10})
		require.NoError(t, err)
		require.Len(t, res, k)
	...
	t.Run("EfZeroUsesDefault", func(t *testing.T) {
		...
		res, err := idx.SearchVectors(context.Background(), query, k, nil, SearchOptions{Ef: 0})
		require.NoError(t, err)
		require.Len(t, res, k)
	...
	t.Run("NegativeEfUsesDefault", func(t *testing.T) {
		...
		res, err := idx.SearchVectors(context.Background(), query, k, nil, SearchOptions{Ef: -1})
		require.NoError(t, err)
		require.Len(t, res, k)
	...
	t.Run("ResultOrder", func(t *testing.T) {
		res, err := idx.SearchVectors(context.Background(), query, k, nil, SearchOptions{Ef: 50})
		require.NoError(t, err)
		require.Len(t, res, k)
