...
func TestFuseRRF_LimitTruncation(t *testing.T) {
	dense := make([]HybridResult, 20)
	for i := 0; i < 20; i++ {
		dense[i] = HybridResult{ID: uint32(i), Score: float64(20 - i)}
	}
...
func TestFuseLinear_PureKeyword(t *testing.T) {
	dense := []HybridResult{{ID: 1, Score: 1.0}}
	sparse := []HybridResult{{ID: 2, Score: 1.0}}
...
func TestFuseLinear_LimitTruncation(t *testing.T) {
	dense := make([]HybridResult, 10)
	sparse := make([]HybridResult, 10)
	for i := 0; i < 10; i++ {
		dense[i] = HybridResult{ID: uint32(i), Score: float64(10 - i)}
		sparse[i] = HybridResult{ID: uint32(i + 10), Score: float64(10 - i)}
	}
...
func TestFuseCascade_EmptyExact(t *testing.T) {
	exactIDs := map[uint32]struct{}{}
	keyword := []HybridResult{{ID: 1, Score: 5.0}}
	vector := []HybridResult{{ID: 2, Score: 0.9}}
...
func TestFuseCascade_Duplicates(t *testing.T) {
	exactIDs := map[uint32]struct{}{1: {}}
	keyword := []HybridResult{{ID: 1, Score: 5.0}}
	vector := []HybridResult{{ID: 1, Score: 0.9}} // Same ID in both
...
func TestDedupeAndSort_LimitTruncation(t *testing.T) {
	input := make([]HybridResult, 20)
	for i := 0; i < 20; i++ {
		input[i] = HybridResult{ID: uint32(i), Score: float64(i)}
	}
...
func BenchmarkFuseCascade(b *testing.B) {
	exactIDs := make(map[uint32]struct{})
	for i := 0; i < 50; i++ {
		exactIDs[uint32(i)] = struct{}{}
	}

	keyword := make([]HybridResult, 100)
	vector := make([]HybridResult, 100)
	for i := 0; i < 100; i++ {
		keyword[i] = HybridResult{ID: uint32(i), Score: float64(100 - i)}
		vector[i] = HybridResult{ID: uint32(i + 25), Score: float64(100 - i)}
	}
...
func BenchmarkDedupeAndSort(b *testing.B) {
	input := make([]HybridResult, 200)
	for i := 0; i < 200; i++ {
		input[i] = HybridResult{ID: uint32(i % 100), Score: float64(i)} // 50% duplicates
	}
