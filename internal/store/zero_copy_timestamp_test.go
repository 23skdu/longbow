...
// ensureTimestampOld implementation for benchmarking (Copy-based)
func ensureTimestampOld(mem memory.Allocator, rec arrow.RecordBatch) (arrow.RecordBatch, error) {
	...
	// Copy path: create new columns
	oldCols := rec.Columns()
	newCols := make([]arrow.Array, len(oldCols)+1)

	// Copy existing columns (Retain)
	...
	now := time.Now()
	val, _ := arrow.TimestampFromTime(now, arrow.Nanosecond)

	tsValues := make([]arrow.Timestamp, rec.NumRows())
	for i := range tsValues {
		tsValues[i] = val
	}
	tsBldr.AppendValues(tsValues, nil)

	newCols[len(oldCols)] = tsBldr.NewArray()
	// No defer Release for newCols[len(oldCols)] because it is passed to NewRecordBatch which retains?
	// Actually NewRecordBatch expects arrays. We transfer ownership or should Release after NewRecordBatch?
	// Standard pattern: NewRecordBatch retains input arrays if needed or expects caller to keep them alive?
	// Documentation: "The returned record batch shares the underlying data..."
	// Actually we should Release newCols[len(oldCols)] after creation if we don't want to leak,
	// OR rely on NewRecordBatch taking it.
	// But let's follow safety: deferred release relative to this scope.
	defer newCols[len(oldCols)].Release()

	// New schema
	fields := make([]arrow.Field, len(schema.Fields())+1)
	copy(fields, schema.Fields())
	fields[len(fields)-1] = arrow.Field{Name: "timestamp", Type: arrow.FixedWidthTypes.Timestamp_ns}
