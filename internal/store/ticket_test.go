...
func TestComplex128_TicketParsing(t *testing.T) {
	...
	// If the server expects flattened []float64, []any (nested) will cause runtime error later.
	// We'll verify what the parser logic actually does with this.
	_, isFlat := ticket3.Vector.([]any)
	assert.True(t, isFlat, "Unmarshal should produce []any")

	// In the nested case:
	// vector is []any containing []any
	list, ok := ticket3.Vector.([]any)
	require.True(t, ok)
