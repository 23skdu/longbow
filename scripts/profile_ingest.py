...
def collect_pprof(url, label, duration=30):
    ...
    try:
        ...
        res = requests.get(
            f"{url}/debug/pprof/profile?seconds={duration}", timeout=duration + 10
        )
...
def benchmark_put_sustained(client, batch_size, duration_sec):
    print(
        f"Starting Sustained Ingestion (Batch={batch_size}, Duration={duration_sec}s)..."
    )
    ...
    pprof_thread = threading.Timer(
        5.0, collect_pprof, args=["http://localhost:9090", "node1", 30]
    )
    pprof_thread.start()
...
def main():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1000, help="Batch size per request"
    )
