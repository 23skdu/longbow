    ...
    while not stop_event.is_set():
        try:
            ...
            if op == "put":
                ...
                req = json.dumps(
                    {"dataset": DATASET_NAME, "vector": vec, "k": 5}
                ).encode("utf-8")
...
def check_memory_pressure():
    try:
        ...
        if available_mb < 512:
            print(
                f"  Memory pressure detected ({available_mb:.0f}MB available), skipping pprof"
            )
            return True
    except Exception:
        pass
        ...
        for i, url in enumerate(pprof_urls):
            ...
            try:
                ...
                if resp.status_code == 200:
                    with open(
                        f"{output_dir}/heap_node{i}_{timestamp}.pprof", "wb"
                    ) as f:
...
def monitor_system(pid_list, start_time, stats):
    ...
    while not stop_event.is_set():
        ...
        p99 = 0.0
        if stats["latencies"]:
            p99 = statistics.quantiles(stats["latencies"][-1000:], n=100)[
                98
            ]  # approx p99 of last 1000

        elapsed = int(time.time() - start_time)
        rss_mb = rss_total / 1024 / 1024

        print(
            f"{datetime.now().isoformat()},{elapsed},{ops},{errs},{p99:.2f},{cpu_total:.1f},{rss_mb:.1f}"
        )
        sys.stdout.flush()
...
def main():
    ...
    parser.add_argument(
        "--uris", default="grpc://localhost:3000", help="Comma-separated Longbow URIs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pprof-urls",
        default="",
        help="Comma-separated pprof URLs (e.g. http://localhost:9090)",
    )
    parser.add_argument(
    ...
    if not pids:
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            if "longbow" in proc.info["name"] or (
                proc.info["cmdline"] and "longbow" in proc.info["cmdline"][0]
            ):
    ...
    if pids:
        ...
        print(
            "No local PIDs found to monitor (assuming remote cluster or not started yet)"
        )
    ...
    if pprof_urls:
        profiler = threading.Thread(
            target=collect_profiles, args=(pprof_urls, args.output_dir)
        )
