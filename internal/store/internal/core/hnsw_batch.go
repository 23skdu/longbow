...
func (h *ArrowHNSW) SearchWithBatchDistance(query []float32, k int) []RankedResult {
	...
	candidateIDs := make([]types.VectorID, 0, len(candidates))

	for _, n := range candidates {
		vec := h.getVectorHelper(types.VectorID(n.ID))
		if vec != nil {
			candidateVectors = append(candidateVectors, vec)
			candidateIDs = append(candidateIDs, types.VectorID(n.ID))
		}
	}
...
func (h *ArrowHNSW) SearchBatchOptimized(queries [][]float32, k int) [][]RankedResult {
	...
	allCandidatesMap := make(map[types.VectorID][]float32)
	queryCandidates := make([][]types.VectorID, len(queries))

	// Stage 1: Gather candidates for all queries
	for qi, query := range queries {
		candidates := h.SearchForParallel(query, expandedK)
		queryCandidates[qi] = make([]types.VectorID, 0, len(candidates))

		for _, n := range candidates {
			vid := types.VectorID(n.ID)
	...
	for qi, query := range queries {
		...
		validIDs := make([]types.VectorID, 0, len(candIDs))
...
func (h *ArrowHNSW) RerankBatch(query []float32, candidateIDs []types.VectorID, k int) []RankedResult {
	...
	validIDs := make([]types.VectorID, 0, len(candidateIDs))
...
func (h *ArrowHNSW) getVectorHelper(id types.VectorID) []float32 {
