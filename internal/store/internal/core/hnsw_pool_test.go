...
func TestHNSW_ScratchPoolBasic(t *testing.T) {
	...
	queryVec := vectors[0]
	result, err := idx.SearchVectors(context.Background(), queryVec, 1, nil, types.SearchOptions{})
	require.NoError(t, err)
	require.NotEmpty(t, result)
	assert.Equal(t, types.VectorID(0), result[0].ID)

	queryVec1 := vectors[1]
	result, err = idx.SearchVectors(context.Background(), queryVec1, 1, nil, types.SearchOptions{})
	require.NoError(t, err)
	require.NotEmpty(t, result)
	assert.Equal(t, types.VectorID(1), result[0].ID)
}
	...
	for g := 0; g < numGoroutines; g++ {
		...
		go func(goroutineID int) {
			...
			for s := 0; s < searchesPerGoroutine; s++ {
				...
				result, err := idx.SearchVectors(context.Background(), queryVec, 5, nil, types.SearchOptions{})
...
func TestHNSW_ScratchPoolDifferentDimensions(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NotEmpty(t, result)
			assert.Equal(t, types.VectorID(0), result[0].ID, "first result should be self")
		})
	}
}
