...
func TestStrategies_Agreement(t *testing.T) {
	...
	// We need a real GraphNavigator for this, backed by a mock/stub types.GraphData.
	// We can use the logic from graph_navigator_test.go (createGraphData) to make a real one.

	gd := types.NewGraphData(10, 2, false, false, -1, false, false, false, types.VectorTypeFloat32, false, false, false, 8, "test", nil)
	// 1->2
