...
# Full CPU Performance Matrix — All DTypes including turboquant
...
OUTFILE="$LOG_DIR/perf_matrix_cpu_${TIMESTAMP}.json"
...
stop_server() {
    ...
    pkill -9 -f "longbow.*bench1" 2>/dev/null || true
    sleep 3
    while lsof -i :3000 2>/dev/null | grep -q LISTEN; do
        pkill -9 -f "longbow" 2>/dev/null || true
        sleep 1
    done
}
...
# Main
echo "======================================================================"
echo "FULL CPU PERFORMANCE MATRIX ($PLATFORM, $(uname -m))"
echo "Started: $(date)"
    ...
    for dtype in "${DTYPE_ARR[@]}"; do
        for count in "${COUNT_ARR[@]}"; do
            ...
            if run_benchmark "$dim" "$dtype" "$count"; then
                ...
                echo "  Server log: $LOG_DIR/longbow_${dtype}_${dim}_${count}.log"
                echo "  Last 30 lines of server log:"
                tail -30 "$LOG_DIR/longbow_${dtype}_${dim}_${count}.log" 2>/dev/null || true
                stop_server
                exit 1
            fi
        done
