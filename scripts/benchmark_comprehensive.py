...
def generate_batch(start_id, count, dim, include_metadata=False):
    ...
    if include_metadata:
        ...
        texts = pa.array(
            [f"document {i} about topic {i % 5}" for i in range(count)],
            type=pa.string(),
        )
        fields.extend(
...
def benchmark_do_put(clients, start_id, count, batch_size=1000):
    ...
    print(
        f"    DoPut: {total} vectors in {duration:.2f}s ({throughput:.0f} vectors/s, {bandwidth_mb:.2f} MB/s, {bandwidth_mbits:.2f} Mb/s)"
    )
...
def benchmark_do_get(clients, num_queries=100):
    ...
    for i in range(num_queries):
        ...
        try:
            ticket = flight.Ticket(
                json.dumps({"dataset": DATASET, "limit": 10}).encode()
            )
    ...
    print(
        f"    DoGet: {total_records} records in {duration:.2f}s ({throughput:.0f} records/s, {bandwidth_gb:.2f} GB/s, {bandwidth_mbits:.2f} Mb/s), Errors: {errors}"
    )
...
def benchmark_do_exchange(clients, num_queries=500):
    ...
    if latencies:
        ...
        print(
            f"    DoExchange: P50={p50:.2f}ms, P95={p95:.2f}ms, P99={p99:.2f}ms, Errors={errors}"
        )
...
def benchmark_dense_search(clients, k=10, num_queries=1000, concurrency=4):
    ...
    def run_query():
        ...
        try:
            ...
            req = json.dumps({"dataset": DATASET, "vector": vec, "k": k}).encode(
                "utf-8"
            )
    ...
    if latencies:
        ...
        p99 = np.percentile(latencies, 99)
        print(
            f"    Dense: P50={p50:.2f}ms, P95={p95:.2f}ms, P99={p99:.2f}ms, Errors={errors}"
        )
        return p50, p95, p99, errors
    return 0, 0, 0, errors
...
def benchmark_sparse_search(clients, k=10, num_queries=500):
    ...
    if latencies:
        ...
        p99 = np.percentile(latencies, 99)
        print(
            f"    Sparse: P50={p50:.2f}ms, P95={p95:.2f}ms, P99={p99:.2f}ms, Errors={errors}"
        )
        return p50, p95, p99, errors
    return 0, 0, 0, errors
...
def benchmark_filtered_search(clients, k=10, num_queries=500):
            ...
            req = json.dumps(
                {
                    ...
                    "k": k,
                    "filters": [
                        {"field": "category", "operator": "==", "value": f"cat_{i % 3}"}
                    ],  # Filter to 30% of data
                }
            ).encode("utf-8")
    ...
    if latencies:
        ...
        p99 = np.percentile(latencies, 99)
        print(
            f"    Filtered: P50={p50:.2f}ms, P95={p95:.2f}ms, P99={p99:.2f}ms, Errors={errors}"
        )
        return p50, p95, p99, errors
    return 0, 0, 0, errors
...
def benchmark_hybrid_search(clients, k=10, num_queries=500):
    ...
    if latencies:
        ...
        p99 = np.percentile(latencies, 99)
        print(
            f"    Hybrid: P50={p50:.2f}ms, P95={p95:.2f}ms, P99={p99:.2f}ms, Errors={errors}"
        )
        return p50, p95, p99, errors
    return 0, 0, 0, errors
...
def benchmark_tombstone_deletion(clients, ids_to_delete):
    ...
    print(
        f"    Deletion: {len(ids_to_delete)} IDs in {duration:.2f}s ({throughput:.0f} deletes/s), Errors={errors}"
    )

    # Verify search after deletion
    time.sleep(1)  # Allow propagation
    print(f"  Verifying search after deletion...")
    p50, p95, p99, search_errors = benchmark_dense_search(
        clients, num_queries=100, concurrency=2
    )
...
def collect_pprof(urls, label=""):
    ...
    try:
        ...
        available_mb = (
            soft / (1024 * 1024) if soft != resource.RLIM_INFINITY else float("inf")
        )
        if available_mb < 512:
            print(f"    Skipping pprof: memory limit too low ({available_mb:.0f}MB)")
            return
    except Exception:
        pass
        ...
        for i, client in enumerate(clients):
            try:
                action = flight.Action(
                    "check_readiness", json.dumps({"dataset": DATASET}).encode("utf-8")
                )
...
def main():
    ...
    parser.add_argument(
        "--uris",
        default="grpc://localhost:3000,grpc://localhost:3010,grpc://localhost:3020",
    )
    parser.add_argument(
        "--pprof",
        default="http://localhost:9090,http://localhost:9091,http://localhost:9092",
    )
    ...
    for target in SIZES:
        ...
        # 1. DoPut
        if needed > 0:
            throughput, bandwidth, bandwidth_mbits, duration = benchmark_do_put(
                clients, current_count, needed
            )
    ...
    results.add(
        "tombstone_deletion", "Delete Throughput (deletes/s)", f"{del_throughput:.0f}"
    )
