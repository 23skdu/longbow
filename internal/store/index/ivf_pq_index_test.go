...
func TestIVFPQIndex_Search(t *testing.T) {
	...
	results, err := idx.SearchInternal(context.Background(), query, 10, nil, types.SearchOptions{})
	require.NoError(t, err)
...
func TestIVFPQIndex_SearchWithK(t *testing.T) {
	...
	for _, k := range []int{1, 5, 10, 50} {
		results, err := idx.SearchInternal(context.Background(), query, k, nil, types.SearchOptions{})
		require.NoError(t, err)
...
func TestIVFPQIndex_DimensionMismatch(t *testing.T) {
	...
	_, err = idx.SearchInternal(context.Background(), []float32{1, 2, 3}, 1, nil, types.SearchOptions{})
	assert.Error(t, err)
}
...
func TestIVFPQIndex_SearchQueryInCluster(t *testing.T) {
	...
	results, err := idx.SearchInternal(context.Background(), query, 20, nil, types.SearchOptions{})
	require.NoError(t, err)
	require.NotEmpty(t, results)
...
func TestIVFPQIndex_SearchWithFilter(t *testing.T) {
	...
	t.Run("Selective Filter", func(t *testing.T) {
		...
		results, err := idx.SearchInternal(context.Background(), query, 10, filter, types.SearchOptions{})
		require.NoError(t, err)
