...
func TestBM25InvertedIndexDelete(t *testing.T) {
	...
	results := idx.SearchBM25("hello", 10, nil, nil)
...
func TestBM25InvertedIndexSearchBM25(t *testing.T) {
	...
	results := idx.SearchBM25("apple", 10, nil, nil)
	if len(results) == 0 {
	...
	results = idx.SearchBM25("banana", 10, nil, nil)
	if len(results) == 0 {
	...
	results = idx.SearchBM25("cherry", 10, nil, nil)
	if len(results) != 1 {
...
func TestBM25InvertedIndexMultiTermQuery(t *testing.T) {
	...
	results := idx.SearchBM25("quick brown", 10, nil, nil)
	if len(results) == 0 {
...
func TestBM25InvertedIndexIDFWeighting(t *testing.T) {
	...
	results := idx.SearchBM25("rare", 10, nil, nil)
	if len(results) != 1 {
	...
	commonResults := idx.SearchBM25("common", 10, nil, nil)
...
func TestBM25InvertedIndexLengthNormalization(t *testing.T) {
	...
	results := idx.SearchBM25("target", 10, nil, nil)
	if len(results) != 2 {
...
func TestBM25InvertedIndexEmptyQuery(t *testing.T) {
	...
	results := idx.SearchBM25("", 10, nil, nil)
	if results != nil {
...
func TestBM25InvertedIndexConcurrency(t *testing.T) {
	...
	for i := 100; i < 150; i++ {
		...
		go func() {
			defer wg.Done()
			idx.SearchBM25("test", 10, nil, nil)
		}()
	}
	wg.Wait()
}
