...
func TestColumnInvertedIndex_IndexColumn(t *testing.T) {
	...
	rows := idx.Lookup("category", "cat1")
	if len(rows) != 2 {
		t.Errorf("Expected 2 rows for cat1, got %d", len(rows))
	}

	rows = idx.Lookup("category", "cat2")
	if len(rows) != 1 {
...
func TestColumnInvertedIndex_MultipleRecords(t *testing.T) {
	...
	idx.IndexRecord(0, rec0, []string{"status"})
	rec0.Release()
	bldr.Release()
	...
	idx.IndexRecord(1, rec1, []string{"status"})
	rec1.Release()
	bldr.Release()

	// Lookup should return positions from both records
	rows := idx.Lookup("status", "active")
	if len(rows) != 3 {
...
func TestColumnInvertedIndex_Int64Column(t *testing.T) {
	...
	idx.IndexRecord(0, rec, []string{"user_id"})

	rows := idx.Lookup("user_id", "100")
	if len(rows) != 2 {
...
func TestColumnInvertedIndex_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(idx_num int) {
			...
			rec := bldr.NewRecordBatch()
			idx.IndexRecord(idx_num, rec, []string{"id"})
			rec.Release()
			bldr.Release()
		}(i)
	}
...
func TestColumnInvertedIndex_GetMatchingRowIndices(t *testing.T) {
	...
	indices := idx.GetMatchingRowIndices(0, "category", "a")
	if len(indices) != 2 {
...
func TestColumnInvertedIndex_HasIndex(t *testing.T) {
	...
	rec := bldr.NewRecordBatch()
	idx.IndexRecord(0, rec, []string{"indexed"}) // Only index "indexed" column
	rec.Release()
	bldr.Release()
...
func BenchmarkColumnInvertedIndex_Lookup(b *testing.B) {
	...
	for recIdx := 0; recIdx < 100; recIdx++ {
		...
		rec := bldr.NewRecordBatch()
		idx.IndexRecord(recIdx, rec, []string{"category"})
		rec.Release()
		bldr.Release()
	}
...
func BenchmarkColumnInvertedIndex_HighCardinality_Lookup(b *testing.B) {
	...
	for i := 0; i < numBatches; i++ {
		...
		bldr.Field(0).(*array.StringBuilder).AppendValues(values, nil)
		rec := bldr.NewRecordBatch()
		idx.IndexRecord(i, rec, []string{"uuid"})
		rec.Release()
		bldr.Release()
	}
