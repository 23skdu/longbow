...
func (s *perpShard) get(k int, trackStats bool) ([]types.VectorID, bool) {
	var ptr any
	var targetCap int
	...
	default:
		...
		return make([]types.VectorID, k), false
	}
	...
	if ptr == nil {
		...
		slice := make([]types.VectorID, k, targetCap)
		return slice, false
	}
...
func (s *perpShard) put(slice []types.VectorID, trackStats bool) {
	if slice == nil {
		return
	}
...
func (p *PerPResultPool) Get(k int) []types.VectorID {
	shardIdx := p.selectShardByK(k)
	slice, _ := p.shards[shardIdx].get(k, p.enableStats)
	return slice
}

// Put returns a []types.VectorID slice to the appropriate pool shard.
func (p *PerPResultPool) Put(slice []types.VectorID) {
	if slice == nil {
		return
	}
