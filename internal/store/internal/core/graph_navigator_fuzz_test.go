...
func FuzzFindPath(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, startID, targetID uint32, maxHops int, earlyTerminate bool, distThreshold float32) {
		...
		g := types.NewGraphData(100, 2, false, false, 0, false, false, false, types.VectorTypeFloat32, false, false, false, 8, "test", nil, false)
		...
		nav := NewGraphNavigator("fuzz", func() *types.GraphData { return g }, config, nil)
		err = nav.Initialize()
		require.NoError(t, err)
