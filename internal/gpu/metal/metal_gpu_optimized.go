...
"    uint offset = gid * dim;\n"
"    \n"
"    // Use SIMD vector loads for 8-way parallelism with half -> float conversion\n"
"    float sum = 0.0f;\n"
"    uint vectorWidth = dim / 8;\n"
"    uint remainder = dim % 8;\n"
"    \n"
"    // Process 8 elements at a time using half8 vector loads\n"
"    for (uint i = 0; i < vectorWidth; i++) {\n"
"        half8 queryVec = half8(query[i*8], query[i*8+1], query[i*8+2], query[i*8+3],\n"
"                                query[i*8+4], query[i*8+5], query[i*8+6], query[i*8+7]);\n"
"        half8 vecVec = half8(vectors[offset + i*8], vectors[offset + i*8+1], vectors[offset + i*8+2], vectors[offset + i*8+3],\n"
"                           vectors[offset + i*8+4], vectors[offset + i*8+5], vectors[offset + i*8+6], vectors[offset + i*8+7]);\n"
"        float8 diff = float8(queryVec) - float8(vecVec);\n"
"        sum += dot(diff, diff);\n"
"    }\n"
"    \n"
"    // Handle remainder\n"
"    for (uint i = 0; i < remainder; i++) {\n"
"        float diff = float(query[vectorWidth * 8 + i]) - float(vectors[offset + vectorWidth * 8 + i]);\n"
"        sum += diff * diff;\n"
"    }\n"
"    \n"
"    distances[gid] = sqrt(sum);\n"
...
"        float v = float(vectors[offset + i]);\n"
"        dotProd += q * v;\n"
"        queryMag += q * q;\n"
"        vecMag += v * v;\n"
"    }\n"
...
"        half8 vecVec = half8(vectors[offset + i*8], vectors[offset + i*8+1], vectors[offset + i*8+2], vectors[offset + i*8+3],\n"
"                           vectors[offset + i*8+4], vectors[offset + i*8+5], vectors[offset + i*8+6], vectors[offset + i*8+7]);\n"
"        sum += dot(float8(queryVec), float8(vecVec));\n"
"    }\n"
"    \n"
"    for (uint i = 0; i < remainder; i++) {\n"
"        sum += float(query[vectorWidth * 8 + i]) * float(vectors[offset + vectorWidth * 8 + i]);\n"
"    }\n"
"    \n"
"    products[gid] = sum;\n"
"}\n"
...
"// ============================================================================\n"
"\n"
"// Warp-level reduction for L2 distance (32-thread warp)\n"
...
"    }\n"
"    \n"
"    // Warp-level reduction (SIMD group)\n"
...
"}\n"
"\n"
"// Fused distance + top-k kernel (reduces memory bandwidth)\n"
"kernel void compute_l2_and_topk_warp(\n"
"    device const float* query [[buffer(0)]],\n"
"    device const float* vectors [[buffer(1)]],\n"
"    device float* topDistances [[buffer(2)]],\n"
"    device int* topIndices [[buffer(3)]],\n"
...
"    float dist = sqrt(sum);\n"
"    \n"
"    // Warp-level reduction to find top-k\n"
"    // Simple approach: each thread writes, then reduction\n"
"    threadgroup float localDists[256];\n"
"    threadgroup uint localIndices[256];\n"
"    \n"
"    localDists[lid] = dist;\n"
"    localIndices[lid] = gid;\n"
"    \n"
"    threadgroup_barrier(mem_flags::mem_threadgroup);\n"
"    \n"
"    // Parallel top-k selection within threadgroup\n"
"    for (uint i = 0; i < k && i < 256; i++) {\n"
"        if (lid == i) {\n"
"            float minDist = localDists[lid];\n"
"            uint minIdx = lid;\n"
"            \n"
"            for (uint j = lid + 1; j < 256; j++) {\n"
"                if (localDists[j] < minDist) {\n"
"                    minDist = localDists[j];\n"
"                    minIdx = j;\n"
"                }\n"
...
"            if (i < k) {\n"
"                topDistances[i] = minDist;\n"
"                topIndices[i] = localIndices[minIdx];\n"
"            }\n"
"            \n"
"            localDists[minIdx] = INFINITY;\n"
"        }\n"
