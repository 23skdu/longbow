...
// Use helper to write Parquet
if err := writeParquet(f, recs); err != nil {
s.logger.Error("Failed to write parquet snapshot", "name", name, "error", err)
f.Close()
continue
}
f.Close()
...
// Open Parquet file
pf, err := parquet.OpenFile(path)
if err != nil {
s.logger.Error("Failed to open snapshot file", "name", name, "error", err)
continue
}

// Read records using helper
recs, err := readParquet(pf, s.mem)
// We can close the file handle now as we read into memory (Arrow records)
pf.Close()
