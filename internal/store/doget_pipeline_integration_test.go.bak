...
func TestDoGetUsesPipelineForMultiBatch(t *testing.T) {
	...
	const numBatches = 10
	records := make([]arrow.Record, numBatches) //nolint:staticcheck
...
func TestDoGetSerialPathForSingleBatch(t *testing.T) {
	...
	err := store.StoreRecords("single_batch", []arrow.Record{record}) //nolint:staticcheck
	if err != nil {
...
// TestDoGetPipelineMetrics verifies Prometheus metrics are exported
func TestDoGetPipelineMetrics(t *testing.T) {
	logger := zap.NewNop()
	mem := memory.NewGoAllocator()
	...
	if stats.BatchesProcessed < 0 {
		t.Error("BatchesProcessed should be initialized")
	}
}

// TestDoGetPipelineThresholdConfig verifies pipeline threshold is configurable
func TestDoGetPipelineThresholdConfig(t *testing.T) {
	logger := zap.NewNop()
	mem := memory.NewGoAllocator()

	store := NewVectorStoreWithPipelineThreshold(mem, logger, 4, 16, 5)
	if store == nil {
		t.Fatal("NewVectorStoreWithPipelineThreshold returned nil")
	}

	threshold := store.GetPipelineThreshold()
	if threshold != 5 {
