...
// PQArrowHNSWConfig holds the configuration for product quantization.
type PQArrowHNSWConfig struct {
	Enabled bool
...
// ArrowPQEncoder handles encoding/decoding using product quantization.
type ArrowPQEncoder struct {
	config   PQArrowHNSWConfig
...
// NewArrowPQEncoder creates encoder from pre-trained codebook.
func NewArrowPQEncoder(cfg PQArrowHNSWConfig, codebook [][][]float32) *ArrowPQEncoder {
...
func (e *ArrowPQEncoder) EnableSDC() {
	m := e.config.M
	k := e.config.Ksub
	if k == 0 { k = 256 }
	...
	for i := 0; i < m; i++ {
		...
		for c1 := 0; c1 < k; c1++ {
			...
			for c2 := 0; c2 < k; c2++ {
				d := arrowSquaredL2(e.codebook[i][c1], e.codebook[i][c2])
...
func (e *ArrowPQEncoder) SDCDistanceOneBatch(codeA, codesB []byte, m int, results, scratch []float32) {
...
// TrainArrowPQEncoder trains codebook via k-means.
func TrainArrowPQEncoder(cfg PQArrowHNSWConfig, vectors [][]float32) (*ArrowPQEncoder, error) {
...
func (e *ArrowPQEncoder) Encode(vec []float32) []byte {
	codes := make([]byte, e.config.M)
	e.EncodeInto(vec, codes)
	return codes
}

func (e *ArrowPQEncoder) EncodeInto(vec []float32, codes []byte) {
...
func (e *ArrowPQEncoder) ComputeTableFlat(query []float32) []float32 {
	mTotal := e.config.M
	ksub := e.config.Ksub
	if ksub == 0 { ksub = 256 }
	...
	for m := 0; m < mTotal; m++ {
		...
		for k, center := range e.codebook[m] {
			...
			d := arrowSquaredL2(subquery, center)
			table[baseIdx+k] = d
		}
	}
	return table
...
func (e *ArrowPQEncoder) ComputeTableFlatInto(query, dst []float32) []float32 {
	mTotal := e.config.M
	ksub := e.config.Ksub
	if ksub == 0 { ksub = 256 }
...
func arrowSquaredL2(a, b []float32) float32 {
	var sum float32
	for i := range a {
		d := a[i] - b[i]
		sum += d * d
