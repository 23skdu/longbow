...
func BenchmarkTurboQuantSearch(b *testing.B) {
	mem := memory.NewGoAllocator()
	dim := 384
	numVectors := 10000

	for _, dtype := range []types.VectorDataType{types.VectorTypeFloat32, types.VectorTypeTQ} {
		b.Run(fmt.Sprintf("Type_%s", dtype), func(b *testing.B) {
			// Create dataset
