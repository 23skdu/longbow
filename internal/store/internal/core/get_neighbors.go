...
func LookupNeighbors(ctx context.Context, ds types.IndexDataProvider, externalID uint64, k int) ([]NeighborResult, error) {
	if ds == nil {
	...
	switch idx := ds.GetIndex().(type) {
	case *ArrowHNSW:
		result, err := arrowHNSWLookupNeighbors(idx, externalID, k)
		elapsed := time.Since(start)
		emitGetNeighborsMetrics(ds.GetName(), "hnsw", err, elapsed, len(result))
		return result, err

	default:
		indexTypeLabel := fmt.Sprintf("%T", ds.GetIndex())
		metrics.GetNeighborsTotal.WithLabelValues(ds.GetName(), indexTypeLabel, "not_supported").Inc()
