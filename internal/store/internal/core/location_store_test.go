...
func TestChunkedLocationStore_ReverseIndex(t *testing.T) {
	...
	// Test Append
	loc1 := types.Location{BatchIdx: 0, RowIdx: 1}
	id1 := s.Append(loc1)
	...
	// Test Set (update)
	locUpdated := types.Location{BatchIdx: 2, RowIdx: 5}
	...
	// Test non-existent
	_, okMissing := s.GetID(types.Location{BatchIdx: 99, RowIdx: 99})
...
func TestChunkedLocationStore_Concurrency(t *testing.T) {
	...
	for i := 0; i < iter; i++ {
		go func(idx int) {
			defer wg.Done()
			loc := types.Location{BatchIdx: idx, RowIdx: idx} // Unique locations
			s.Append(loc)
		}(i)
	}
	wg.Wait()
	...
	for i := 0; i < iter; i++ {
		loc := types.Location{BatchIdx: i, RowIdx: i}
		id, ok := s.GetID(loc)
