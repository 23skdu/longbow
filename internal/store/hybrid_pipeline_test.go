...
func TestFuseLinearPureVector(t *testing.T) {
dense := []HybridResult{{ID: 1, Score: 1.0}}
sparse := []HybridResult{{ID: 2, Score: 1.0}}

// alpha=1.0 means pure vector
results := FuseLinear(dense, sparse, 1.0, 5)

// Only dense result should have weight
for _, r := range results {
if r.ID == 2 && r.Score > 0 {
			_ = r // sparse result included as expected
// This is fine - sparse might still appear with 0 weight component
}
}
}

func TestFuseCascade(t *testing.T) {
exactIDs := map[uint32]struct{}{1: {}, 2: {}}
keyword := []HybridResult{{ID: 1, Score: 5.0}, {ID: 3, Score: 4.0}}
vector := []HybridResult{{ID: 2, Score: 0.9}, {ID: 4, Score: 0.8}}
...
dense := make([]HybridResult, 100)
sparse := make([]HybridResult, 100)
for i := 0; i < 100; i++ {
dense[i] = HybridResult{ID: uint32(i), Score: float64(100 - i)}
sparse[i] = HybridResult{ID: uint32(i + 50), Score: float64(100 - i)}
}
