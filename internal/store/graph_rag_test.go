...
func TestGraphRAG_SearchHybrid(t *testing.T) {
	...
	results, err := s.SearchHybrid(ctx, datasetName, query, "", 5, 0.1, 0, 0.5, 2)
	require.NoError(t, err)
	assert.NotEmpty(t, results)

	// Case 2: alpha=1 (Full GraphRAG re-ranking)
	results, err = s.SearchHybrid(ctx, datasetName, query, "", 5, 1.0, 0, 0.5, 2)
	require.NoError(t, err)
	assert.NotEmpty(t, results)
}
...
func TestGraphRAG_Stability_Large(t *testing.T) {
	...
	start := time.Now()
	results, err := s.SearchHybrid(ctx, datasetName, query, "", 10, 1.0, 0, 0.5, 10)
	require.NoError(t, err)
