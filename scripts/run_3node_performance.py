...
def start_cluster():
    ...
    p1 = subprocess.Popen(
        ["./longbow"], env=env1, stdout=open("node1.log", "w"), stderr=subprocess.STDOUT
    )
    nodes.append(p1)
    ...
    p2 = subprocess.Popen(
        ["./longbow"], env=env2, stdout=open("node2.log", "w"), stderr=subprocess.STDOUT
    )
    nodes.append(p2)
    ...
    p3 = subprocess.Popen(
        ["./longbow"], env=env3, stdout=open("node3.log", "w"), stderr=subprocess.STDOUT
    )
    nodes.append(p3)
    ...
    try:
        ...
        if os.path.exists(output_file):
            with open(output_file, "r") as f:
                ...
                search = next(
                    (r for r in data if r["name"].startswith("VectorSearch")), None
                )
                return {
...
def main():
    ...
    try:
        ...
        for dtype in DTYPES:
            res = run_bench(dtype)
            results.append({"dtype": dtype, "dim": DIM, **res})

        print("\n" + "=" * 80)
        print(
            f"{'DType':<12} | {'Dim':<6} | {'Put MB/s':<10} | {'Get MB/s':<10} | {'Search QPS':<10}"
        )
        print("-" * 80)
        for r in results:
            print(
                f"{r['dtype']:<12} | {r['dim']:<6} | {r['put_mb']:<10.2f} | {r['get_mb']:<10.2f} | {r['search_qps']:<10.2f}"
            )
        print("=" * 80)
