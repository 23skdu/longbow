...
ds.Index.Graph.Add(hnsw.MakeNode(VectorID(j), vec))
}
store.vectors.Set(ds.Name, ds)
}
...
ds1.Index.Graph.Add(hnsw.MakeNode(VectorID(0), []float32{1, 2, 3, 4}))
store.vectors.Set(ds1.Name, ds1)

// Dataset without index
ds2 := &Dataset{Name: "without_index"}
store.vectors.Set(ds2.Name, ds2)
