...
type InsertContext struct {
	// types.Candidate lists for neighbor search at each layer
		...
		pool: sync.Pool{
			New: func() any {
				return &InsertContext{
					candidates: make([][]types.Candidate, types.ArrowMaxLayers),
					visited:    types.NewArrowBitset(10000),
					neighbors:  make([]types.Candidate, 0, 100),
				}
			},
		},
	}
