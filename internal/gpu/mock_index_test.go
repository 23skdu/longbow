...
func TestMockIndex_SearchFloat16(t *testing.T) {
	cfg := DefaultGPUConfig()
	cfg.Dimension = 3

	idx := NewMockIndex(cfg, BackendCPU)
	defer idx.Close()
	...
	for i, v := range queryF32 {
		queryF16[i] = float32ToFloat16Mock(v)
	}
...
func TestMockIndex_SearchComplex64(t *testing.T) {
	cfg := DefaultGPUConfig()
...
func TestMockIndex_SearchComplex128(t *testing.T) {
	cfg := DefaultGPUConfig()
...
// float32ToFloat16Mock converts float32 to uint16 float16 representation
func float32ToFloat16Mock(v float32) uint16 {
