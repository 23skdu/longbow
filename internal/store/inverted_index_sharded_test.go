...
func TestShardedInvertedIndex_BasicAddSearch(t *testing.T) {
	...
	addToShardedIndex(idx, VectorID(2), "error code 404 not found")
	addToShardedIndex(idx, VectorID(3), "success operation completed")
	addToShardedIndex(idx, VectorID(4), "error fatal crash dump")
...
func TestShardedInvertedIndex_ConcurrentSearch(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		addToShardedIndex(idx, VectorID(i), text)
	}
...
func TestShardedInvertedIndex_ConcurrentMixed(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for j := 0; j < 50; j++ {
				addToShardedIndex(idx, VectorID(500+id*50+j), fmt.Sprintf("new doc %d", id*50+j))
				addCount.Add(1)
			}
		}(i)
	}
