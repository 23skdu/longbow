...
// TestDatasetCachedSize tests Dataset-level size caching
func TestDatasetCachedSize(t *testing.T) {
	alloc := memory.NewGoAllocator()
	schema := arrow.NewSchema([]arrow.Field{
		{Name: "id", Type: arrow.PrimitiveTypes.Int64},
	}, nil)
	...
	// Should be cached now - call again
	totalSize2 := ds.TotalSize()
	if totalSize != totalSize2 {
		t.Errorf("Dataset size inconsistent: %d vs %d", totalSize, totalSize2)
	}
}

// TestDatasetInvalidateSizeCache tests cache invalidation on record changes
func TestDatasetInvalidateSizeCache(t *testing.T) {
	alloc := memory.NewGoAllocator()
	schema := arrow.NewSchema([]arrow.Field{
		{Name: "id", Type: arrow.PrimitiveTypes.Int64},
	}, nil)
	...
	ds := &Dataset{
		Name:    "test",
		Records: []arrow.RecordBatch{rec1},
	}

	size1 := ds.TotalSize()

	// Add another record
	builder2 := array.NewRecordBuilder(alloc, schema)
	defer builder2.Release()
	builder2.Field(0).(*array.Int64Builder).AppendValues([]int64{2, 3, 4, 5}, nil)
	...
	size2 := ds.TotalSize()
	if size2 <= size1 {
		t.Errorf("Expected larger size after adding record: %d vs %d", size1, size2)
	}
}
