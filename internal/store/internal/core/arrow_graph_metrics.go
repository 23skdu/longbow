...
func (h *ArrowHNSW) AnalyzeGraph() GraphMetrics {
	...
	layer := 0
	visited := types.NewArrowBitset(nodeCount)
...
func (h *ArrowHNSW) bfsComponentSize(data *types.GraphData, layer int, startNode uint32, visited *types.ArrowBitset) int {
	...
	for len(queue) > 0 {
		...
		// Iterate neighbors
		cID := types.ChunkID(curr)
		cOff := types.ChunkOffset(curr)
...
func (h *ArrowHNSW) bfsDiameter(data *types.GraphData, layer int, startNode uint32) int {
	nodeCount := int(h.nodeCount.Load())
	visited := types.NewArrowBitset(nodeCount)
	visited.Set(int(startNode))
