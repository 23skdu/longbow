...
func NewSchemaEvolutionManager(initialSchema *arrow.Schema) *SchemaEvolutionManager {
	...
	// Copy fields from initial schema
	fields := make([]arrow.Field, initialSchema.NumFields())
	for i := 0; i < initialSchema.NumFields(); i++ {
		fields[i] = initialSchema.Field(i)
		// Track column metadata
		mgr.columns[fields[i].Name] = &ColumnMetadata{
			Name:      fields[i].Name,
			Type:      fields[i].Type,
			AddedAt:   1,
			DroppedAt: 0,
		}
		mgr.columnOrder = append(mgr.columnOrder, fields[i].Name)
	}
	...
	// Update metrics
	metrics.SchemaVersionCurrent.Set(1)
...
func (m *SchemaEvolutionManager) AddColumn(name string, dtype arrow.DataType) error {
	...
	// Update metrics
	metrics.SchemaVersionCurrent.Set(float64(newVer))
