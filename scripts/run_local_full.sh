...
for MODE in "${MODES[@]}"; do
    echo "Starting Local Benchmark: Mode=$MODE"
    python3 -u scripts/unified_benchmark.py --mode $MODE --dtypes "$DTYPES" --dims "$DIMS" --counts "$COUNTS" --memory $MEMORY --pprof --workers $WORKERS --queries $QUERIES --label "local_$MODE"
done
