...
func TestDoGetPipelineCreation(t *testing.T) {
	tests := []struct {
		...
		{"default workers", 0, 0, 4}, // 0 means use default (mocked as 4 here or runtime.NumCPU)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Use >= 1 for default workers check if we can't control runtime.NumCPU
			if tt.workers == 0 {
				if pipeline.NumWorkers() <= 0 {
					t.Errorf("NumWorkers() = %d, want > 0", pipeline.NumWorkers())
				}
			} else if pipeline.NumWorkers() != tt.wantWorkers {
